...
const API_URL = import.meta.env.PROD
  ? "https://chat-ai-application-backend.onrender.com/api"
...
export const channelAPI = {
  ...
  joinWithCode: (inviteCode) =>
    api.post("/channels/join-with-code", { inviteCode }),
  inviteUser: (id, userId) => api.post(`/channels/${id}/invite`, { userId }),
  acceptInvite: (id) => api.post(`/channels/${id}/accept-invite`),
  promoteModerator: (id, userId) =>
    api.post(`/channels/${id}/promote-moderator`, { userId }),
  promoteAdmin: (id, userId) =>
    api.post(`/channels/${id}/promote-admin`, { userId }),
...
export const messageAPI = {
  ...
  search: (query, channelId, page = 1, limit = 20) =>
    api.get(
      `/messages/search?query=${query}&channelId=${channelId || ""}&page=${page}&limit=${limit}`,
    ),
...
export const userAPI = {
  getAll: (page = 1, limit = 10) =>
    api.get(`/users/all-users?page=${page}&limit=${limit}`),
  ...
  uploadPicture: (formData) =>
    api.post("/users/upload-picture", formData, {
